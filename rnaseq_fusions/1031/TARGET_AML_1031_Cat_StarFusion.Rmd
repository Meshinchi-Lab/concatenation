...
#   
#   files <- dir(path=path.fus, pattern="*.out")
#   filenames=paste(path.fus, files,sep="/")
#   
#   # test <- gsub("^.+/([BPR].+[0-9]R|S-1327)\\.out", "\\1", filenames)
#   # print(tail(test))
#   regex="^.+/([BPR].+[0-9]R|S-1327)\\.out"
# 
#   fusionCaller <- catRbind(filenames=filenames, regex=regex, header=TRUE)
#   print(length(unique(fusionCaller$Patient)))
#   
#   filename <- paste0("TARGET_AML_1031_", dir,"_06.05.18.csv")
...
```{r}
CDE.RBD <- read.csv("~/reference_mapping-files/TARGET_AML_1031_0531_RBD_master_list_CDEs.csv", 
                       stringsAsFactors = FALSE)

head(CDE.RBD) #likely not clean enough
# dim(CDE.RBD) #1565   83
```
...
```{r}
star <- read.csv("Concatenated/TARGET_AML_1031_starfusion_06.05.18.csv", 
                 stringsAsFactors = FALSE)

# dim(star) #145,025 rows by 12
head(star)
```

```{r}
length(table(star$X.Fusion)) #29,937 unique fusion partners 
length(unique(star$Patient)) #1,107. Missing 4 samples due to inability to process the FQ.
```
...
dim(star.fmt) #145025 by    16
```

```{r}
head(star.fmt)
# write.csv(star.fmt, "TARGET_AML_1031_STAR_Fusion_reformatted_7.02.2018.csv")
```
...
# NBM
dim(NBM) #5,707 fusions detected
```
...
```{r}
dim(star.fmt.filter) #74054  by  14
length(unique(star.fmt.filter$USI)) #1,107 samples
```

```{r}
head(star.fmt.filter)
# write.csv(star.fmt.filter, "TARGET_AML_1031_STAR_Fusion_reformatted_FilteredForNBM_7.02.2018.csv", row.names = FALSE)
```
...
star.fmt.primary <- star.fmt.filter %>% 
  ...
  select(Patient,USI,Group,Type,Fusion.Category,
         ...
         Fusion.Detected.STAR,-Primary.Fusion)

  
dim(star.fmt.primary) #67623  by  10 (this reduced the file by 6,431 lines)
# length(unique(star.fmt.primary$Patient)) #1107
```
...
#Add in Age and Clinical Information


```{r}
star.fmt.primary <- star.fmt.primary %>%
  # mutate(USI=as.character(USI)) %>%
  left_join(., select(merged, TARGET.USI.1, Age.Yrs,ISCN), by=c("USI"="TARGET.USI.1")) %>%
  mutate(Age.Yrs=round(Age.Yrs, digits=2)) %>%
  mutate(Fusion.Category=ifelse(grepl("None", Fusion.Category), "",Fusion.Category)) %>%
  select(Patient,USI,Age.Yrs,ISCN, everything()) %>%
  arrange(Group)


dim(star.fmt.primary) #67623  by  20

```

```{r}
head(star.fmt.primary)
# write.csv(star.fmt.primary, "TARGET_AML_STAR_Fusion_reformatted_FilteredForNBM_PrimaryFusions_7.02.2018.csv")
```
...
dim(inter) # 23293  by  19
# write.csv(inter, "TARGET_AML_STAR_Fusion_reformatted_InterChromosomal_FilteredForNBM_PrimaryFusions_7.02.2018.csv")
```
...
dim(intra) # 44395  by  19

# write.csv(intra, "TARGET_AML_STAR_Fusion_reformatted_IntraChromosomal_FilteredForNBM_PrimaryFusions_7.02.2018.csv")
```
...
# write.csv(FusionGenes, "TARGET_AML_1031_STAR_Fusion_FilteredForNBM_ByType_7.02.2018.csv")
```
...
head(Inter.G.Tab)
# write.csv(Inter.G.Tab, "TARGET_AML_1031_STAR_Fusion_reformatted_InterChromosomal_FilteredForNBM_GeneLevel_Tables_07.02.2018.csv", row.names = FALSE)
```
...
# write.csv(Intra.G.Tab, "TARGET_AML_1031_STAR_Fusion_reformatted_IntraChromosomal_FilteredForNBM_GeneLevel_Tables.csv", row.names = FALSE)
```
...
```{r}
star.fmt.primary <- read.csv("Reformatted/TARGET_AML_1031_STAR_Fusion_reformatted_FilteredForNBM_PrimaryFusions_7.02.2018.csv", row.names = 1)
...
```{r}
# write.table(ForAnno, "TARGET_AML_1031_Starfusion_inTopHapFmt_ForAGFusion.txt", 
