...
selected <- c(1,16,9,15)
```

```{r}
cated <- catExpnData(filenames = allfiles, regex = pattern, cols = selected, header = FALSE, removeFirstLine = FALSE)
names(cated) <- c("gene_id","geneSymbol", "FractionalCounts", "RPKM")
sapply(cated, dim)
```
...
```{r}
all(apply(cated$gene_id, 2, function(x) identical(x=x, y=cated$gene_id[,1]))) #all are identical order. 
```
...
withCols <- lapply(cated[toAddCols], function(x) addCols(df = x, symbol = cated$geneSymbol[,1], id = cated$gene_id[,1]))
...
for (i in 1:3){
    f <- withCols[[i]] %>%
      select(geneSymbol, gene_id, matches("Kas|MV4"),
             matches("MPN[0-9]", ignore.case = TRUE), 
             matches("sorted", ignore.case = TRUE), everything())
    
    filename <- paste0("TARGET_AML_0531_1031_Ribodepleted_RNAseq_",data.types[i],".csv")
